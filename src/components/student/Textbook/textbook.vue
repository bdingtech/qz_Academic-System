<template>
  <div class="textbook">
      ...
      <div class="textbook_content">
        ...
        <template v-else>
          <RouterLink v-for="(item,index) in textbooks" :key="index" :to="{path:'/student/textbook_Details',query:{info:JSON.stringify(item)}}">
  ...
  export default {
    ...
    methods:{
      ...
      async searchBook(){
        ...
        const res = await Api.getTeachingMaterial(params)
      ...
      async findCourse(val){
        ...
        const res = await Api.getTeachingCourse(params)
        let concatList = []
...
// WEBPACK FOOTER //
// src/components/student/Textbook/textbook.vue