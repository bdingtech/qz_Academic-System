  ...
  <div>
    ...
    <div class="eva-detail">
        ...
        <div class="eva-rate" v-if="evaluateIndex == index">
          <ul>
            <div v-for="(oItem,index) in item.optionData" :key="oItem.val">
              <li :class="index == calc(oItem.isxz,index) ? 'checked' : ''" @click="pickOpt(oItem.isxz,index)">{{index+1}}.{{oItem.zbmc}}</li>
            </div>
          </ul>
        </div>
      </div>
      ...
      <template v-if="this.evaluateIndex + 1 > this.textIndex">
        <div v-for="(item,index) in textEvaluate" :key="item.val">
...
// WEBPACK FOOTER //
// src/components/teacher/live/supervise.vue